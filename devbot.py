...
bot = commands.Bot('Xf ', description=description)
...
@bot.command()
async def Search(ctx, searchentry: str):
    r = requests.get('https://api.duckduckgo.com/?q='+searchentry+'&format=json')
    j=r.json()
    da=j[0]["RelatedTopics"]
    daicon=da["Icon"]
    embed=discord.Embed(title=j["Heading"], description=j["AbstractSource"],color=0x5C5D7F)
    embed.set_author(name="Synapse", url="https://github.com/KingRegera", icon_url=daicon["URL"])
    embed.set_thumbnail(url=daicon["URL"])
    embed.add_field(name="Current Price", value=da["Text"], inline=False)
    embed.add_field(name="Open Price", value=da["Result"], inline=False)
    embed.add_field(name="High Price", value=da["h"], inline=False)
    embed.add_field(name="Low Price", value=da["FirstURL"], inline=False)
